...
def build_binding(
    ...
    for name, rule in DATA_SOURCES.items():
        sources[name] = _provided(rule, result)
        parts = [] if sources[name] else [_why(c, result.check(c)) for c in _failing(rule, result)]
        if rule.get("note"):
    ...
    return {
        ...
        "probe": {
            ...
            "unbound": unbound,
            "manual_checks": [_manual(m, result, tools) for m in MANUAL_CHECKS],
        },
    }


def _provided(rule: Mapping[str, Any], result: ProbeResult) -> bool:
    """A source is provided when any check of `any_of` holds, or every check of `all_of` does."""
    any_of: list[str] = rule.get("any_of", [])
    all_of: list[str] = rule.get("all_of", [])
    return any(result.check(c).ok for c in any_of) or bool(
        all_of and all(result.check(c).ok for c in all_of)
    )


def _failing(rule: Mapping[str, Any], result: ProbeResult) -> list[str]:
    """The checks that kept a source from being provided, for the note that says why."""
    any_of: list[str] = rule.get("any_of", [])
    all_of: list[str] = rule.get("all_of", [])
    return any_of + [c for c in all_of if not result.check(c).ok]


def _manual(
    check: Mapping[str, str], result: ProbeResult, tools: Mapping[str, str]
) -> dict[str, Any]:
    """One manual check as this tenant needs it.

    `automated` says the source behind it can be read and the deployment has the feature that
    writes it, so the tool named in `tool` is worth calling. `required` says the portal must be
    read by hand regardless: the table holds no rows, so the tool answers nothing, and nothing but
    the Action center says whether the feature acted. It is not required only where the table holds
    rows, or where the deployment stated the feature absent, or the licence does not expose it.
    """
    found = result.check(check["automated_by"])
    return {
        **{k: v for k, v in check.items() if k != "operation"},
        "tool": tool_ref(tools[check["operation"]]),
        "status": found.status,
        "automated": found.ok,
        "required": found.status not in ("available", "unlicensed", "not_exposed"),
    }
